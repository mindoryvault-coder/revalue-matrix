...
function looksLikeAddress(query) {
  return /[0-9]/.test(query) || ["시", "군", "구", "동", "읍", "면", "로", "길", "번지", " 산 "].some((token) => query.includes(token));
}

function maskSecretText(text, env) {
  let out = String(text || "");
  for (const value of [env?.SITE_ACCESS_TOKEN, env?.VWORLD_API_KEY, env?.MOLIT_BUILDING_API_KEY, env?.DATA_GO_KR_API_KEY]) {
...
async function fetchJson(url, params, env) {
  ...
  try {
    return { status: response.status, data: JSON.parse(text), url: maskSecretText(requestUrl.toString(), env) };
  } catch {
    return {
...
async function searchBuildings(query, env) {
  if (!query.trim()) return { ok: false, query, candidates: [], logs: ["검색어가 없습니다."] };
  const order = looksLikeAddress(query) ? ["address", "place"] : ["place", "address"];
  const logs = [];
  let candidates = [];
  for (const type of order) {
    const result = await vworldSearch(query, type, env);
  ...
  candidates = dedupeCandidates(candidates)
    ...
    .slice(0, 10);
  return { ok: candidates.length > 0, query, candidates, logs };
}
...
function modules(data, buildingInfo, types, programs, cost, comparison) {
  ...
  return [
    ...
    { name: "02 지역 맥락", value: types[0], decision: programs.slice(0, 2).join(", "), description: "지역 수요, 건물 유형, 장소성을 연결해 운영 가능한 재생 프로그램을 제안합니다." },
    { name: "03 구조 진단", value: `${risk.toFixed(1)}점`, decision: risk >= 70 ? "정밀진단 우선" : risk >= 45 ? "보강 병행" : "재생 검토 가능", description: "구조, 설비, 누수 위험을 먼저 잡고 재생 설계의 안전 범위를 정합니다." },
    { name: "04 경제 판단", value: `보수 ${repair.growth ?? "-"}% / 재생 ${regen.growth ?? "-"}%`, decision: `기준 사업비 ${formatKrw(cost.base)}`, description: "보수 중심과 건축재생 디자인의 투자비, 수익 증가율, 회수기간을 비교합니다." },
    { name: "05 설계 보조", value: `${buildingInfo.previous_use || "기존 용도"} 기반 ${types[0]}`, decision: identity >= 70 ? "장소성 반영 설계" : "파일럿 후 구체화", description: "평가 결과를 콘셉트, 공간 개입, 프로그램 브리프로 번역합니다." },
  ];
}
