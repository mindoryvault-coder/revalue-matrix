...
async function enrichSelectedCandidate(candidate, seq) {
  ...
  try {
    ...
    const hasPnu = Boolean(data.building?.pnu);
    const hasHub = Boolean(data.hub_summary && Object.keys(data.hub_summary).length);
    const hasCommercial = Boolean(data.commercial_summary && data.commercial_summary.store_count !== undefined);
    if (data.title_record_found) {
      setAutoFillStatus("건축물대장 표제부 정보로 보정 창을 자동 채웠습니다.");
      setSearchStatus("공공데이터 자동 보정이 완료되었습니다. 필요한 값만 수정한 뒤 분석을 누르세요.");
    } else if (data.record_found || hasPnu || hasHub || hasCommercial) {
      const parts = [];
      if (hasPnu) parts.push("PNU");
      if (hasHub) parts.push("건축HUB 상세");
      if (hasCommercial) parts.push("상권정보");
      setAutoFillStatus(`${parts.join(", ")} 기준으로 일부 자동 보정했습니다. 표제부가 없으면 준공연도/층수/연면적은 직접 확인이 필요합니다.`);
      setSearchStatus("일부 공공데이터가 연결되었습니다. 비어 있는 값만 직접 보정해 주세요.");
    } else {
